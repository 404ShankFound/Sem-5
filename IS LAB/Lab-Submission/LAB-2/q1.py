...
def decrypt(cipher, keys):
    ...
    for i in range(0, len(cipher), 64):
        block = cipher[i:i+64]
        plain += des_block(block, keys, True)

    # Remove zero padding
    while plain.endswith('00000000'):
        plain = plain[:-8]

    return bits_to_text(plain)

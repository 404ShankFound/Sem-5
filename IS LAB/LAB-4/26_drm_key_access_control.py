"""Question 26: Drm key access control

Build an ElGamal DRM service with content encryption, authorized and time-limited access, revocation, renewal and audit logging.
"""
...
class DRMService:
    ...
    def add_content(self, content_id, title, content, allowed_users, valid_minutes=30):
        keys = elgamal_keygen()
        ...
        self.logs.append(f"add -> {content_id}")
...
def main():
    print("==============================")
    print("DRM KEY ACCESS CONTROL")
    print("==============================")
    print()
    drm = DRMService()
    drm.add_content("song1", "Studio Track", "STREAM-ONLY", ["Alice", "Bob"], valid_minutes=0)
    drm.renew("song1", 30)
    print("Alice access:", drm.get_content("song1", "Alice"))
    try:
        print("Eve access:", drm.get_content("song1", "Eve"))
    except Exception as exc:
        print("Denied:", exc)
    drm.revoke_user("song1", "Bob")
    try:
        print("Bob access after revoke:", drm.get_content("song1", "Bob"))
    except Exception as exc:

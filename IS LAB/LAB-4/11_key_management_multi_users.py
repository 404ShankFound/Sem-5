"""Question 11: Key management multi users

Generate and manage separate public/private keys for multiple entities and support adding new entities.
"""
...
class KMS:
    ...
    def get_public(self, entity):
        return self.records[entity]["keys"]["public"]


def main():
    print("==============================")
    print("KEY MANAGEMENT MULTI USERS")
    print("==============================")
    print()
    kms = KMS()
    ...
    print("Initial Public Keys:")
    for user in ["Alice", "Bob", "Carol"]:
        print(user, "->", kms.get_public(user))
    print()

    print("Add New Entity: Dave")
    kms.add_entity("Dave")
    print("Dave ->", kms.get_public("Dave"))
    print()

    print("Revoke Bob and show state:")
    kms.revoke("Bob")
    for user, record in kms.records.items():
        print(user, "revoked:", record["revoked"], "public:", record["keys"]["public"])
    print()
